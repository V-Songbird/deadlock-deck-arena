...
The hub and the games run in Spanish and English. The switch is in the top bar, and `?lang=en`
works on any page.
...
later change to a game — currently only the Spanish/English language layer — sits in commits after
it, so the raw output stays recoverable.
